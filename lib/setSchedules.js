  ...
  garoon
    ...
    .then(schedules => {
      ...
      if (schedules.length === 0) { // 予定がない場合
        return cron.schedule(`30 9 ${day} ${month} *`, () => robot.send(envelope, "本日の予定は...ありませぬ(。。)"));
      }
      ...
      cron.schedule(`30 9 ${day} ${month} *`, () => robot.send(envelope, `<!channel>\n${text}`));
    })
    .catch(err => {
      robot.send(envelope, `<!channel>\nエラー発生...\nパスワードの変更期限が迫った可能性がありそう`)
      robot.send(envelope, `${err.response}`)
    })
}
